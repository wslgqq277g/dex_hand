...
def generate_free_robot_hand_info() -> Dict[str, FreeRobotInfo]:
    inspire_hand_free_info = FreeRobotInfo(path="robot/inspire_hand_description/inspirehand_free.urdf", dof=18,
                                          palm_name="righthand_baselink")
    ...
    info_dict = dict(inspire_hand_free=inspire_hand_free_info,
...
def generate_retargeting_link_names(robot_name):
    if "shadow_hand" in robot_name or "adroit_hand" in robot_name:
        ...
        link_names = ["righthand_baselink", "thtip", "fftip", "mftip", "rftip", "lftip"]
